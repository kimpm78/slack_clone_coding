...
router.post("/workspaces", isLoggedIn, async (req, res, next) => {
  ...
  try {
    ...
    if (exWorkspace) {
      await t.rollback();
      return res.status(404).send("すでに使用中のワークスペースURLです。");
    }
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      if (workspace.Channels.find((v) => v.name === req.body.name)) {
        await t.rollback();
        return res.status(404).send("すでに存在するチャンネル名です。");
      }
      const channel = await Channel.create(
        {
          name: req.body.name,
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      if (!channel) {
        return res.status(404).send("存在しないチャンネルです。");
      }
      return res.json(channel);
    } catch (error) {
      next(error);
...
try {
  ...
  console.error("uploads フォルダがないため uploads フォルダを 生成します。");
...
router.post(
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      if (!user) {
        await t.rollback();
        return res.status(404).send("存在しない利用者です。");
      }
...
router.post("/users", isNotLoggedIn, async (req, res, next) => {
  try {
    ...
    if (exUser) {
      return res.status(403).send("すでに使用中のIDです。");
    }
